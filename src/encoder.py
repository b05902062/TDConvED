...
class TDconv(nn.Module):
	...
	def forward(self,x):
		...
		x_filter=x_filter.reshape(x_filter.shape[0],1,-1)#return batch*1*(encode_dim*g_sample) 3d tensor.
		x_filter=self.deform_conv(x_filter)#return batch*g_encode_kernel*g_sample
		...
		a=torch.tensor([o  for i in range(0,g_sample) for o in range(i-math.floor(g_encode_kernel/2),i+math.floor(g_encode_kernel/2)+1)]).type(torch.cuda.FloatTensor).reshape(1,-1,1).expand(x_filter.shape).cuda()
		b=torch.from_numpy(np.arange(g_sample)).type(torch.cuda.FloatTensor).reshape(1,1,-1).expand(x_filter.shape).cuda()
...
class TDconvE(nn.Module):
	...
	def forward(self,x):
		...
		x=self.TDenc1(x)#return batch*g_sample*encode_dim
		x=self.TDenc2(x)#return batch*g_sample*encode_dim
		if unsqueeze==True:
			x=x.squeeze(dim=1)
		return x	
